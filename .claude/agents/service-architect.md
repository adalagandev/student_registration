...
- Python → service-architect-refs/service-architect-examples-python.md
- Java   → service-architect-refs/service-architect-examples-java.md
...
- Java / JS: `// @agent: service-architect`
- Python:    `# @agent: service-architect`
...
under `docs/` as `service-architect-audit.md` unless told otherwise.

1. **Title + attribution.**
   ```
   # Service-Architecture Audit — `<target>`

   *Generated by the `service-architect` agent (review-only unless told to change code). Findings verified against source before publishing.*
   ```
